...
// Batch submission size: accumulate this many iocbs before calling io_submit
...
impl LibaioEngine {
    ...
    /// Flush queued iocbs to the kernel in a single batched io_submit call
    fn flush_submissions(&mut self) -> Result<()> {
        ...
        let nr = self.submit_queue.len();

        // Build pointer array into pre-allocated iocbs (stable, never reallocated after init)
        let mut iocb_ptrs: Vec<*mut IoControlBlock> = Vec::with_capacity(nr);
        for i in 0..nr {
            let idx = self.submit_queue[i];
            iocb_ptrs.push(&mut self.iocbs[idx] as *mut IoControlBlock);
        }
        ...
        if result < 0 {
            ...
            // Roll back: return all iocbs to pool and remove from pending_ops
            for &idx in &self.submit_queue {
                let user_data = self.iocbs[idx].data;
                self.pending_ops.remove(&user_data);
                self.available_iocbs.push(idx);
            }
            self.submit_queue.clear();
            return Err(err).context(format!("io_submit batch of {} failed", nr));
        }

        let submitted = result as usize;
        if submitted < nr {
            // Partial submission: keep unsubmitted indices in queue for next flush
            self.submit_queue.drain(..submitted);
        } else {
            self.submit_queue.clear();
        }
...
impl IOEngine for LibaioEngine {
    fn init(&mut self, config: &EngineConfig) -> Result<()> {
        ...
        self.submit_queue = Vec::with_capacity(config.queue_depth);
    ...
    fn poll_completions(&mut self) -> Result<Vec<IOCompletion>> {
        ...
        // Flush any queued iocbs before waiting for completions
        self.flush_submissions()?;
        ...
        for i in 0..num_events {
            ...
            // Find and return the iocb to the pool using event.obj (pointer to iocb)
            let iocb_ptr = event.obj as *const IoControlBlock;
            let base_ptr = self.iocbs.as_ptr();
            let idx = unsafe { iocb_ptr.offset_from(base_ptr) as usize };
    ...
    fn cleanup(&mut self) -> Result<()> {
        if let Some(ctx) = self.ctx {
            // Flush any queued but unsubmitted iocbs
            self.flush_submissions()?;
