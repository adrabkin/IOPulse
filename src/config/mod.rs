...
impl WorkloadConfig {
    ...
    /// queue depth, and IO mode.
    ///
    /// Registered buffers and fixed files are restricted to O_DIRECT mode because:
    /// - In buffered mode the kernel page cache handles address translation; the
    ///   `register_buffers` quiescence overhead (unregister + re-register on every
    ///   queue drain) outweighs the benefit and causes a measurable IOPS regression.
    /// - O_DIRECT bypasses the page cache, so pinning user-space buffers and
    ///   pre-registering fds yield their full benefit.
...
mod tests {
    ...
    #[test]
    fn test_workload_to_engine_config_io_uring_high_qd_direct() {
        // O_DIRECT + io_uring + high QD: optimizations enabled
        let workload = WorkloadConfig {
            read_percent: 100,
            write_percent: 0,
            read_distribution: vec![],
        ...
        assert!(engine_config.use_registered_buffers); // Enabled for io_uring + O_DIRECT + QD >= 32
        assert!(engine_config.use_fixed_files);
    }

    #[test]
    fn test_workload_to_engine_config_io_uring_high_qd_buffered() {
        // Buffered io_uring: optimizations disabled to avoid register_buffers quiescence overhead
        let workload = WorkloadConfig {
            read_percent: 100,
            write_percent: 0,
            read_distribution: vec![],
        ...
        let engine_config = workload.to_engine_config();
        assert_eq!(engine_config.queue_depth, 64);
        assert!(!engine_config.use_registered_buffers); // Disabled for buffered IO
        assert!(!engine_config.use_fixed_files);        // Disabled for buffered IO
    }
